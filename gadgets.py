                ...
                for s in r:
                    ...
                    for j, _ in enumerate(hexdata):
                        ...
                        if str(hexdata[j : j + 2]) in retHex:
                            ...
                            gadget = hexdata[j  - (int(sys.argv[3]) * 2 * 16) : j ]  # gadget = hexdata[0 : 30]. Ici on multiplie par 2 le length
                            ...
                            if instList  and flag == 1:
                                counterRet+=1
                                print "gadget %d : \n" % counterRet
                                for ( mnemonic, op_str) in instList[- int(sys.argv[3]):]: #J 'affiche length -1 dernier valeur
                                    print ("  %s %s \n") % (mnemonic, op_str)
